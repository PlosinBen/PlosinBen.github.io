...
export default {
    setup() {
        ...
        const clean_winner = (i) => {
            if (confirm("是否要清除中獎名單")) {
                for (let i in awards) {
                    if (awards[i].winners.length > 0) {
                        awards[i].winners = []
                    }
...
return x - Math.floor(x);">Sin</label>
                ...
                <div class="space-y-2">
                    <div class="flex border-b text-center">
                        ...
                        <div>
                            <button class="bg-gray-200 px-2 py-0.5 rounded-sm" @click="pick_all">全部開獎</button>
                        </div>
                    </div>
                    ...
                    <div class="flex" v-for="(award, i) in awards">
                        ...
                        <div class="flex-grow flex justify-between pr-4" v-if="award.winners.length === 0">
                            <button class="bg-gray-200 px-2 py-0.5 rounded-sm" @click="pick(i)">抽獎</button>
                            <button class="bg-gray-200 px-2 py-0.5 rounded-sm" @click="remove_award(i)">刪除獎項</button>
                        </div>
                    </div>
                </div>

                <div>
                    <button @click="clean_winner">清除中獎名單</button>
                </div>
            </div>
        </div>
    `
